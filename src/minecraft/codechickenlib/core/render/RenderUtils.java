...
import codechickenlib.core.lighting.LC;
...
public class RenderUtils
{
    ...
    public static void renderBlock(Cuboid6 c, int sideMask, Vector3 pos, LightMatrix lightMatrix, IFaceRenderer r)
    {
        double x1 = c.min.x;
        double x2 = c.max.x;
        double y1 = c.min.y;
        ...
        if((sideMask&1) == 0)
        {
            ...
            r.renderFace(face, c.min.y > 0 ? 6 : 0, pos, lightMatrix);
        }
        ...
        if((sideMask&2) == 0)
        {
            ...
            r.renderFace(face, c.max.y < 1 ? 7 : 1, pos, lightMatrix);
        }
        ...
        if((sideMask&4) == 0)
        {
            ...
            r.renderFace(face, c.min.z > 0 ? 8 : 2, pos, lightMatrix);
        }
        ...
        if((sideMask&8) == 0)
        {
            ...
            r.renderFace(face, c.max.z < 1 ? 9 : 3, pos, lightMatrix);
        }
        ...
        if((sideMask&0x10) == 0)
        {
            ...
            r.renderFace(face, c.min.x > 0 ? 10 : 4, pos, lightMatrix);
        }
        ...
        if((sideMask&0x20) == 0)
        {
            ...
            face[3].set(x2, y1, z2, u2, v2);
            r.renderFace(face, c.max.x < 1 ? 11 : 5, pos, lightMatrix);
        }
    }

    public static void renderBlock(Cuboid6 bounds, int sideMask, Vector3 pos, LightMatrix lightMatrix, final int colour, final IUVTransformation u)
    {
        renderBlock(bounds, sideMask, pos, lightMatrix, new IFaceRenderer(){
            @Override
